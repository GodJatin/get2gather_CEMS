...
export default function DateTimePicker({ label, value, onChange, type }: DateTimePickerProps) {
    ...
    // Internal state for calendar navigation (independent of selected value)
    const [viewDate, setViewDate] = useState(new Date());
    
    // Internal state for clock
    const [clockMode, setClockMode] = useState<'hour' | 'minute'>('hour');

    // Initialize viewDate when opened or value changes
    useEffect(() => {
        if (value && type === 'date') {
    ...
    const renderDatePicker = () => {
        ...
        const handleDateClick = (d: number) => {
            // Create date in local time, avoiding timezone issues
    ...
    const ClockPicker = () => {
        ...
        // Clock face radius
        const radius = 80;
        const center = 100; // Center of the 200x200 SVG
        ...
        const renderNumbers = () => {
            const items = clockMode === 'hour' ? 12 : 12; // Both show 12 positions
            
            return Array.from({ length: items }).map((_, i) => {
                let num = i + 1;
                if (clockMode === 'minute') num = i * 5;
                if (clockMode === 'minute' && i === 0) num = 0; 
                if (clockMode === 'minute' && i === 12) num = 60;
        ...
        const handleManualChange = (type: 'hour' | 'minute', valStr: string) => {
            ...
            if (type === 'hour') {
                ...
                let newH = val;
                if (val === 12) newH = 0; // Default to 0 for calc
                if (isPM) newH += 12;
                if (!isPM && val === 12) newH = 0; // 12 AM is 0
                if (isPM && val === 12) newH = 12; // 12 PM is 12
                
                // Simpler logic:
                // If AM: 12->0, 1-11->1-11
                // If PM: 12->12, 1-11->13-23
                let finalH = val;
                if (isPM) {
            ...
            <div className="p-6 w-72 flex flex-col items-center select-none">
                    ...
                    {inputMode === 'clock' ? (
                        <>
                            <button 
                                ...
                                className={`text-4xl font-bold p-2 rounded-xl transition-colors ${
                                    clockMode === 'hour' ? 'text-purple-400 bg-purple-500/10' : 'text-neutral-500'
                                }`}
                            >
                            ...
                            <button 
                                ...
                                className={`text-4xl font-bold p-2 rounded-xl transition-colors ${
                                    clockMode === 'minute' ? 'text-pink-400 bg-pink-500/10' : 'text-neutral-500'
                                }`}
                            >
                        ...
                        <>
                             <input 
                                ...
                                className="w-20 text-4xl font-bold p-2 rounded-xl bg-neutral-800 border border-purple-500 text-center focus:outline-none text-white"
                            />
                            <span className="text-4xl text-neutral-600 pb-2">:</span>
                            <input 
                                type="number" 
                                ...
                                className="w-20 text-4xl font-bold p-2 rounded-xl bg-neutral-800 border border-pink-500 text-center focus:outline-none text-white"
                            />
                        </>
                    )}
                ...
                {inputMode === 'clock' ? (
                    <div className="relative w-[200px] h-[200px] rounded-full bg-neutral-800/50 border border-white/5 shadow-inner">
                        ...
                        {/* Hand Removed as per request */}
                        {/* <div className="absolute top-1/2 left-1/2 h-[80px] w-0.5 bg-purple-500..." /> */}
    ...
    return (
        <div className="relative" ref={containerRef}>
            ...
            <button
                ...
                className={`w-full px-4 py-3 rounded-xl bg-neutral-800/50 border border-white/10 text-left flex items-center justify-between transition-all ${
                    isOpen ? 'border-purple-500 ring-1 ring-purple-500/50' : 'hover:border-white/20'
                }`}
            >
            ...
            <AnimatePresence>
                {isOpen && (
                    ...
                    >
                        {type === 'date' ? renderDatePicker() : <ClockPicker />}
                    </motion.div>
                )}
            </AnimatePresence>
        </div>
