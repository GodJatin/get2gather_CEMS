...
export default function AdminUsersPage() {
    ...
    const handleDelete = async (id: number) => {
        ...
        try {
            ...
            setTotal(t => t - 1);
            toast.success("User deleted successfully and removed from all records");
        } catch (error) {
                    ...
                    <div className="flex bg-neutral-900 border border-white/10 p-1 rounded-xl w-max mx-auto">
                        {tabs.map(tab => (
                            <button
                                key={tab.id}
                                onClick={() => setFilter(tab.id as any)}
                                className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all whitespace-nowrap ${filter === tab.id
                                        ? 'bg-blue-600 text-white shadow-lg shadow-blue-900/20'
                                        : 'text-neutral-400 hover:text-white'
                                    }`}
                            >
                        ...
                        <tbody className="divide-y divide-white/5">
                            {filteredUsers.length === 0 ? (
                                ...
                                <tr key={user.id} className="hover:bg-white/5 transition-colors">
                                    ...
                                    <td className="p-6">
                                        <span className={`px-3 py-1 rounded-full text-xs font-bold border ${user.role === 'admin' ? 'bg-red-500/10 text-red-400 border-red-500/20' :
