...
export default function OrganizerScanPage() {
    ...
    const parseEventDate = (dateStr: string, timeStr: string): Date => {
        // Clean date/time logic (same as before)
        let year, month, day;
    ...
    const checkScanEligibility = () => {
        ...
        try {
            ...
            if (isNaN(eventDateTime.getTime())) throw new Error("Invalid Date");
            ...
            if (diffHours > 2) {
                 setCanScan(false);
                 setTimeMessage(`Starts in ${Math.floor(diffHours)}h ${Math.floor((diffMs % (3600000)) / 60000)}m`);
            } else if (diffHours < -12) {
                 setCanScan(false);
                 setTimeMessage('Event Ended');
            } else {
                setCanScan(true);
                setTimeMessage('Check-in Open');
            }
        } catch (error) {
            setCanScan(false);
            setTimeMessage('Invalid Time');
        }
    };
    ...
    const categorizedEvents = {
        active: events.filter(e => {
            try {
                ...
                const now = new Date();
                const end = new Date(dt.getTime() + 12 * 3600000);
                const start = new Date(dt.getTime() - 2 * 3600000); // Active 2h before
        ...
        upcoming: events.filter(e => {
            try {
                const dt = parseEventDate(e.date, e.time);
                const now = new Date();
                return now < (new Date(dt.getTime() - 2 * 3600000));
            } catch { return false; }
        }),
        completed: events.filter(e => {
             try {
                const dt = parseEventDate(e.date, e.time);
                const now = new Date();
                return now >= (new Date(dt.getTime() + 12 * 3600000));
            } catch { return false; }
        })
    };
