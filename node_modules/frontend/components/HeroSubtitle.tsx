...
const TypewriterText = ({ text, delay = 0 }: { text: string; delay?: number }) => {
  ...
  }, [text, delay, isDeleting]); 

  return (
    <span className="inline-flex items-center">
      {displayText}
      <span className={`w-0.5 h-6 ml-1 bg-accent ${showCursor ? 'opacity-100' : 'opacity-0'}`}>&nbsp;</span>
    </span>
  );
};
  ...
  return (
    <div className="text-xl md:text-2xl text-neutral-400 mb-10 leading-relaxed min-h-[6rem] flex flex-col items-center justify-start">
      ...
      <motion.p
        ...
        className="font-bold bg-gradient-to-r from-primary via-accent to-secondary bg-clip-text text-transparent text-2xl md:text-3xl text-center"
      >
